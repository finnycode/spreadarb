...
end_index = 2000
# Data Loading
aapl_data = pd.read_csv('AAPL_data.csv')[:start_index]
spy_data = pd.read_csv('XLK_data.csv')[:start_index]

aapl_data_untrained = pd.read_csv('AAPL_data.csv')[start_index:end_index]
spy_data_untrained = pd.read_csv('SPY_data.csv')[start_index:end_index]
...
def plot_trading_signals(data_aapl, data_spy, pairs_data):
    ...
    # Filtering out indices that don't exist in the main data
    valid_indices = pairs_data.index.intersection(data_aapl.index)

    # For AAPL data
    axs[0].plot(data_aapl.loc[valid_indices, 'Close'], label='AAPL Close Price', color='black')
    long_aapl_entries = pairs_data[pairs_data['aapl_position'] == 1].index.intersection(valid_indices)
    short_aapl_entries = pairs_data[pairs_data['aapl_position'] == -1].index.intersection(valid_indices)
    long_aapl_exits = pairs_data[(pairs_data['aapl_position'] == 1) & (pairs_data['aapl_daily_pl'] != 0)].index.intersection(valid_indices)
    short_aapl_exits = pairs_data[(pairs_data['aapl_position'] == -1) & (pairs_data['aapl_daily_pl'] != 0)].index.intersection(valid_indices)
    ...
    for entry, exit in zip(long_aapl_entries, long_aapl_exits):
        ...
        entry_counter += 1
        axs[0].text(entry, data_aapl.loc[entry, 'Close'], str(entry_counter), fontsize=9, verticalalignment='bottom', horizontalalignment='right')
        axs[0].text(exit, data_aapl.loc[exit, 'Close'], str(entry_counter), fontsize=9, verticalalignment='bottom', horizontalalignment='right')
    ...
    for entry, exit in zip(short_aapl_entries, short_aapl_exits):
        ...
        entry_counter += 1
        axs[0].text(entry, data_aapl.loc[entry, 'Close'], str(entry_counter), fontsize=9, verticalalignment='top', horizontalalignment='right')
        axs[0].text(exit, data_aapl.loc[exit, 'Close'], str(entry_counter), fontsize=9, verticalalignment='top', horizontalalignment='right')

    axs[0].set_title('AAPL Trading Signals')
    axs[0].set_ylabel('Price')

    # For SPY data
    valid_indices = pairs_data.index.intersection(data_spy.index)
    axs[1].plot(data_spy.loc[valid_indices, 'Close'], label='SPY Close Price', color='black')
    long_spy_entries = pairs_data[pairs_data['spy_position'] == 1].index.intersection(valid_indices)
    short_spy_entries = pairs_data[pairs_data['spy_position'] == -1].index.intersection(valid_indices)
    long_spy_exits = pairs_data[(pairs_data['spy_position'] == 1) & (pairs_data['spy_daily_pl'] != 0)].index.intersection(valid_indices)
    short_spy_exits = pairs_data[(pairs_data['spy_position'] == -1) & (pairs_data['spy_daily_pl'] != 0)].index.intersection(valid_indices)
    ...
    for entry, exit in zip(long_spy_entries, long_spy_exits):
        ...
        entry_counter += 1
        axs[1].text(entry, data_spy.loc[entry, 'Close'], str(entry_counter), fontsize=9, verticalalignment='bottom', horizontalalignment='right')
        axs[1].text(exit, data_spy.loc[exit, 'Close'], str(entry_counter), fontsize=9, verticalalignment='bottom', horizontalalignment='right')
    ...
    for entry, exit in zip(short_spy_entries, short_spy_exits):
        ...
        entry_counter += 1
        axs[1].text(entry, data_spy.loc[entry, 'Close'], str(entry_counter), fontsize=9, verticalalignment='top', horizontalalignment='right')
        axs[1].text(exit, data_spy.loc[exit, 'Close'], str(entry_counter), fontsize=9, verticalalignment='top', horizontalalignment='right')
...
def modified_backtest_strategy(data_aapl, data_spy, upper_thresh, lower_thresh, atr_window, risk_reward_ratio,
    ...
    for idx, row in pairs_data.iterrows():
        ...
        if aapl_open_position:
            pairs_data.loc[idx, 'aapl_daily_pl'] = (data_aapl.loc[idx, 'Close'] - aapl_open_position[
                'entry_price']) / aapl_open_position['entry_price'] * 100 * aapl_open_position['position']
        if spy_open_position:
            pairs_data.loc[idx, 'spy_daily_pl'] = (data_spy.loc[idx, 'Close'] - spy_open_position['entry_price']) / \
                                                  spy_open_position['entry_price'] * 100 * spy_open_position[
                                                      'position']
...
def plot_cumulative_pnl(pairs_data):
    """Plot cumulative P&L for both AAPL and SPY"""
    plt.figure(figsize=(12, 6))
    pairs_data['total_cum_pl'].plot(label="Total Cumulative P&L")
    print(3)
    pairs_data['aapl_cum_pl'].plot(label="AAPL Cumulative P&L", linestyle="--")
    print(4)
    pairs_data['spy_cum_pl'].plot(label="SPY Cumulative P&L", linestyle=":")
    print(5)
...
def main_cli():
    ...
    pairs_data = modified_backtest_strategy(aapl_data_untrained.copy(), spy_data_untrained.copy(), best_upper_thresh, best_lower_thresh, best_atr_window, best_rr_ratio)
    plot_trading_signals(aapl_data_untrained, spy_data_untrained, pairs_data)
    plot_cumulative_pnl(pairs_data)

# Running the main function again to include the cumulative P&L plot
main_cli()